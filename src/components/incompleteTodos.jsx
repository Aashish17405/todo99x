...
const Incompletetodos = () => {
  ...
  const fetchTodos = (async () => {
    ...
    try {
      ...
      toast.error("Failed to fetch incomplete todos");
    } finally {
      setIsLoading(false);
    }
  });
  ...
  return (
    <div>
      <h1 className='ml-24 mt-20 font-extrabold text-2xl sm:ml-36 sm:mt-24 sm:font-extrabold sm:text-5xl'>Todo99x</h1>
      <CreateTodos />
      ...
      <div>
        {todos && todos.map((tod) => (
          <div key={tod._id} className="m-10 flex items-center">
            ...
            <button onClick={() => deleteTodo(tod._id)} className="sm:rounded-full sm:px-2 sm:py-1 hover:bg-blue hover:text-black hover:border-black">
      ...
      <button className='px-2 py-2 bg-black text-xs sm:mt-2  sm:text-base border border-gray-300 sm:px-4 rounded-full sm:ml-56 hover:bg-red hover:text-black hover:border-black' onClick={(e) => logout({ logoutParams: { returnTo: window.location.origin } })}>Logout</button>
      {isLoading && (
